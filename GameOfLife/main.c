...
RGBQUAD **pixelArray(FILE *F, BITMAPINFOHEADER infoHeader) {
    ...
    //считываем байты цветов (с верхнего левого угла до правого нижнего)
...
//Проверка на "живую" клетку (чёрный пиксель)
    ...
    for (int k = 0; k < max_iter; k++) {
        for (int i = 0; i < infoHeader.biHeight; i++) {
            ...
            for (int j = 0; j < infoHeader.biWidth; j++) {
                ...
                if (pixels[i][j] == 1) {
                    //...но кол-во соседей не удовлетворяет условию, то "убиваем" эту клетку,..
